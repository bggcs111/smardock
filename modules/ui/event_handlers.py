...
import config
from modules.indexing.embedder import Embedder, EmbeddingError
...
CONFIRM_REQUIRED_MESSAGE = "删除不可恢复。请先勾选「我已确认要执行删除」，再点击删除按钮。"

# 空状态：一句话，无插图
STATUS_EMPTY = "还没有文档。先从左侧上传一份 PDF 或 Word，系统会自动解析并建立索引。"
REFS_EMPTY = "_提问后，这里会列出引用来源。_"
SUMMARY_EMPTY = "_等待提问_"
DOC_STATUS_EMPTY = "_解析引擎或分块策略更新后，需要重建索引才能生效。_"
SESSION_EMPTY = "当前：**新对话**（提问后自动创建会话）"
...
class KnowledgeQAApp:
    ...
    def handle_upload(
        ...
        if not kb_name:
            yield self._pack(self._list_state(selected, scope), status_md="请先选择上传目标知识库。")
            return
        if not files:
            yield self._pack(
                self._list_state(selected, scope), status_md="请先选择要上传的 PDF / Word 文件。"
            )
            return
        ...
        for index, file_path in enumerate(files, start=1):
            ...
            yield self._pack(
                self._list_state(selected, scope), status_md=self._render_logs(logs, index - 1, total)
            )
            ...
            yield self._pack(
                self._list_state(selected, scope), status_md=self._render_logs(logs, index, total)
            )
        ...
        self.metadata.clear_cached_answers()
        yield self._pack(
            self._list_state(selected, scope), status_md=self._render_logs(logs, total, total)
        )
    ...
    def handle_rebuild(
        ...
        yield self._pack(
            self._list_state(selected, scope),
            doc_status_md=self._render_logs(logs, total, total),
        )
    ...
    def handle_query(
        ...
        def emit(
            ...
            """
            if streaming:
                return (hist, "", *(_update() for _ in range(len(QUERY_KEYS) - 2)))
            return (
                hist,
                "",
                *self._session_updates(current["session_id"]),
                refs_md,
                _update(value=downloads or None, visible=bool(downloads)),
                summary,
            )
        ...
        yield emit([dict(item) for item in display], summary="正在检索相关片段…")

        doc_ids = [scoped_doc["doc_id"]] if scoped_doc else None
        # 隐私模式下用脱敏后的提问做检索：与库内占位符一致，召回反而更准
        hits = self.retriever.retrieve(route.kbs, query_text, doc_ids=doc_ids)
        if not hits:
